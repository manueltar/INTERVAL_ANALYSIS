  ...
  if(length(SELECTED_VARS_UPDATED) >0)
  {
    ...
    if(dim(VEP_CSQ_sel_CIS)[1] >0)
    {
      ...
      if(length(ENSG_array) >0)
      {
        ...
        filename<-paste("DE_LM_HET_RESULTS_NOMINAL_",SELECTED_VARS_UPDATED,'.rds', sep='')
        ...
        if (file.exists(filename)) {
          ...
          if(dim(Results_Nominal_sel)[1] >0)
          {
            ...
            if(dim(MT_set)[1] >0)
            {
              saveRDS(file="DE_CIS_gene.rds", MT_set)  
            }
        ...
        if (file.exists(filename)) {
          ...
          if(dim(Results_Nominal_sel)[1] >0)
          {
            ...
            if(dim(MT_set)[1] >0)
            {
              ...
              saveRDS(file="DE_Block.rds", MT_set)  
            }
...
MT_correction_genome_wide = function (option_list)
{
  ...
  if(length(SELECTED_VARS_UPDATED) >0)
  {
    ...
    if (file.exists(filename)) {
      ...
      DE_genome_wide<-MT_set[which(MT_set$ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS >= 1.3),]
      ...
      if(dim(MT_set)[1] >0)
      {
        saveRDS(file="DE_genome_wide.rds", MT_set)  
      }
