  ...
  if(length(SELECTED_VARS_UPDATED) >0)
  {
    ...
    if (file.exists(filename_rds_VAR)){
      ...
      for(i in 1:length(ENSG_array))
      {
        ...
        if(n_transcripts_per_gene > 1)
        {
          ...
          if(Condition_DEBUG == 1)
          {
            ...
            cat(sprintf(as.character(names(summary(Transposed_Isoform_Expression_sel.m_ADAPTED$Genotype)))))
            cat("\n")
            cat(sprintf(as.character(summary(Transposed_Isoform_Expression_sel.m_ADAPTED$Genotype))))
            cat("\n")
            
          }
          
          Transposed_Isoform_Expression_sel.m_ADAPTED_HET<-Transposed_Isoform_Expression_sel.m_ADAPTED[(as.numeric(Transposed_Isoform_Expression_sel.m_ADAPTED$Genotype) <=2),]
          
          
          
          if(Condition_DEBUG == 1)
          {
            cat("Transposed_Isoform_Expression_sel.m_ADAPTED_HET_0\n")
            cat(str(Transposed_Isoform_Expression_sel.m_ADAPTED_HET))
            cat("\n")
            
            cat("sample_id\n")
            cat(str(unique(Transposed_Isoform_Expression_sel.m_ADAPTED_HET$sample_id)))
            cat("\n")
            
            cat(sprintf(as.character(names(summary(Transposed_Isoform_Expression_sel.m_ADAPTED_HET$Genotype)))))
            cat("\n")
            cat(sprintf(as.character(summary(Transposed_Isoform_Expression_sel.m_ADAPTED_HET$Genotype))))
            cat("\n")
            
          }
         
          
          Transposed_Isoform_Expression_sel.m_ADAPTED_HET.dt<-data.table(Transposed_Isoform_Expression_sel.m_ADAPTED_HET,
                                                             key=c("Genotype","transcript_id"))
          
          if(Condition_DEBUG == 1)
          {
            cat("Transposed_Isoform_Expression_sel.m_ADAPTED_HET.dt_2\n")
            cat(str(Transposed_Isoform_Expression_sel.m_ADAPTED_HET.dt))
            cat("\n")
          }
          
          ### calculate median TPM per genotype
          
          Summary_table<-as.data.frame(Transposed_Isoform_Expression_sel.m_ADAPTED_HET.dt[, .(median=round(as.numeric(summary(TPM)[3]),3)),
                                                                              by=key(Transposed_Isoform_Expression_sel.m_ADAPTED_HET.dt)],stringsAsFactors=F)
      ...
      if(length(list_ACCEPTED_ENST) >0)
      {
        ...
        saveRDS(file=paste("DTU_PASS_Transcripts_",SELECTED_VARS_UPDATED_sel,'.rds', sep=''), TRANSCRIPTS_table_FILTERED_FINAL)
...
LogRatio_LM_model = function (option_list)
{
  ...
  if(length(SELECTED_VARS_UPDATED) >0)
  {
    ...
    if (file.exists(filename_rds_VAR)){
      ...
      TRANSCRIPTS_table_FILTERED_FINAL<-readRDS(file=paste("DTU_PASS_Transcripts_",SELECTED_VARS_UPDATED_sel,'.rds', sep=''))
      ...
      filename<-paste("DTU_LogRatioLM_HET_RESIDUALS_",SELECTED_VARS_UPDATED_sel,'.csv',sep='')
      ...
      if (file.exists(filename)){
        ...
        unlink(filename)
        cat(paste("transcript_id",paste(sample_id_vector, collapse=","), sep=","),
            file=filename,sep="\n")
      ...
      ENSG_array<-ENSG_array[which(ENSG_array%in%c("ENSG00000181652"))]
      ...
      for(i in 1:length(ENSG_array))
      {
        ...
        if(n_transcripts_per_gene > 1)
        {
          ...
          Transposed_Isoform_Expression_sel.m_REDUCED<-Transposed_Isoform_Expression_sel.m[which(Transposed_Isoform_Expression_sel.m$sample_id%in%INTERVAL_covariates_and_PEER_factors_sel$sample_id),]
          ...
          if(Condition_DEBUG == 1)
          {
            ...
            cat("sample_id\n")
            cat(str(Transposed_Isoform_Expression_sel.m_REDUCED$sample_id))
            cat("\n")
            
            cat("transcript_id\n")
            cat(str(Transposed_Isoform_Expression_sel.m_REDUCED$transcript_id))
            cat("\n")
          ...
          for(k in 1:length(ENST_array))
          {
            ...
            row.names(Ratio_df_ENST_sel_HET_LM)<-Ratio_df_ENST_sel_HET$sample_id
            ...
            unselected_columns<-c("sample_id","transcript_id","sum_GENE_EXP","TPM","Reference_ratio_value","scaled_ratio","Genotype","logscaled_ratio")
            ...
            if(Condition_DEBUG == 1)
            {
              ...
              cat("Ratio_df_ENST_sel_HET_LM_REDUCED_LM\n")
              cat(str(Ratio_df_ENST_sel_HET_LM))
              cat("\n")
            ...
            model<-lm(Ratio ~ ., data=Ratio_df_ENST_sel_HET_LM)
      ...
      if(length(list_RESULT) >0)
      {
        ...
        saveRDS(Results_uncorrected,file=paste("DTU_LogLM_HET_RESULTS_NOMINAL_",SELECTED_VARS_UPDATED_sel,'.rds', sep=''))
