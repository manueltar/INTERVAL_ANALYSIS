...
collate_results_function = function(option_list)
{
  ...
  if(length(SELECTED_VARS_UPDATED) >0)
  {
    ...
    for(i in 1:length(SELECTED_VARS_UPDATED))
    {
      ...
      if (file.exists(filename)) {
        ...
        DTU_genome_wide_SIG<-DTU_genome_wide[which(DTU_genome_wide$ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS >= 1.3),]
        ...
        {
          DTU_genome_wide_SIG$VAR<-SELECTED_VARS_sel
    ...
    if (file.exists(filename)) {
      ...
      DTU_BLOCK_SIG<-DTU_BLOCK[which(DTU_BLOCK$ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS >= 1.3),]
      ...
      if(dim(DTU_BLOCK)[1] >0)
      {
        ...
        DTU_BLOCK$VAR<-SELECTED_VARS_sel
    ...
    if (file.exists(filename)) {
      ...
      DTU_CIS_gene_SIG<-DTU_CIS_gene[which(DTU_CIS_gene$ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS >= 1.3),]
      ...
      if(dim(DTU_CIS_gene)[1] >0)
      {
        ...
        DTU_CIS_gene$VAR<-SELECTED_VARS_sel
    ...
    }# i 1:length(SELECTED_VARS_UPDATED)
  ...
  if(length(list_CIS) >0)
  {
    ...
    indx.int<-c(which(colnames(Results_CIS) == "VAR"),which(colnames(Results_CIS) == "ensembl_gene_id"),which(colnames(Results_CIS) == "HGNC"),
                which(colnames(Results_CIS) == "transcript_id"),which(colnames(Results_CIS) == "ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS"),which(colnames(Results_CIS) == "coefficient_Genotypes_specific_CELL_COUNTS"),
  ...
  if(length(list_Block) >0)
  {
    ...
    indx.int<-c(which(colnames(Results_Block) == "VAR"),which(colnames(Results_Block) == "ensembl_gene_id"),which(colnames(Results_Block) == "HGNC"),
                which(colnames(Results_Block) == "transcript_id"),which(colnames(Results_Block) == "ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS"),which(colnames(Results_Block) == "coefficient_Genotypes_specific_CELL_COUNTS"),
    ...
    colnames(Results_Block_subset)[which(colnames(Results_Block_subset) == "coefficient_Genotypes_specific_CELL_COUNTS")]<-"Block_PCHiC_n_breakdown_string"
    
    
    
    Results_DEF<-merge(Results_DEF,
                       Results_Block_subset,
                       by=c("VAR","ensembl_gene_id","HGNC","transcript_id"),
                       all=T)
  ...
  if(length(list_GW) >0)
  {
    ...
    indx.int<-c(which(colnames(Results_GW) == "VAR"),which(colnames(Results_GW) == "ensembl_gene_id"),which(colnames(Results_GW) == "HGNC"),
                which(colnames(Results_GW) == "transcript_id"),which(colnames(Results_GW) == "ajusted.minuslogpvalue_Genotypes_specific_CELL_COUNTS"),which(colnames(Results_GW) == "coefficient_Genotypes_specific_CELL_COUNTS"),
  ...
  if(dim(Results_DEF)[1] >0)
  {
    ...
    write.table(file="DTU_LM_logRatio_results_Main_VARS.tsv", Results_DEF, sep="\t", row.names = F,quote = F)
    write.table(file="DTU_LM_logRatio_results_SIG_collapsed_Main_VARS.tsv", DEF_SIG, sep="\t", row.names = F,quote = F)
